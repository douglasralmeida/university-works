...
void TArvoreBNo_Imprimir(TArvoreBNo No, int nivel, TFuncaoImprimir FuncaoImprimir)
{
	unsigned short i;
	...
	for (i = 0; i < No->Contador; i++)
		FuncaoImprimir(No->Itens[i]);
	putchar('\n');
	nivel++;
	for (i = 0; i <= No->Contador; i++)
		TArvoreBNo_Imprimir(No->Subarvores[i], nivel, FuncaoImprimir);
}
...
bool TArvoreBNo_Inserir(TArvoreB* Arvore, TArvoreBNo NoAtual, void** Item, TArvoreBNo* NoRetorno, void** ItemRetorno)
{
	...
	while (i < NoAtual->Contador && Arvore->FuncaoComparar(NoAtual->Itens[i-1], *Item))
		i++;
	/*-- INSERIR AQUI UMA FUNCAO DE VERIFICAR SE OS ITENS SAO IGUAIS --*/
	
	/*-- passou da saída, volta uma casa --*/
	if (Arvore->FuncaoComparar(*Item, NoAtual->Itens[i-1]))
		i--;
...
void* TArvoreBNo_Pesquisar(TArvoreBNo No, void* Item, TFuncaoComparar FuncaoComparar, TFuncaoIguais FuncaoIguais)
{
	unsigned short i = 1;

	if (No == NULL)
		return NULL;
	while (i < No->Contador && FuncaoComparar(No->Itens[i-1], Item))
		i++;
	if (FuncaoIguais(No->Itens[i-1], Item))
	{
		return No->Itens[i-1];
	}
	if (FuncaoComparar(Item, No->Itens[i-1]))
		return TArvoreBNo_Pesquisar(No->Subarvores[i-1], Item, FuncaoComparar, FuncaoIguais);
	else
		return TArvoreBNo_Pesquisar(No->Subarvores[i], Item, FuncaoComparar, FuncaoIguais);
}
...
{
	TArvoreBNo_Imprimir(Arvore->Raiz, 1, FuncaoImprimir);
}
...
{
	return TArvoreBNo_Pesquisar(Arvore->Raiz, Item, Arvore->FuncaoComparar, Arvore->FuncaoIguais);
}
